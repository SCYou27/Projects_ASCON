...
class Template:
  def __init__(self, func):
    ...
    for frag in range(0, (8*self.Size)):
      Sname = fname+'_scov_s'+str(frag).zfill(3)+'.npy'
      Scov = svm.Load(Sname)
      matS = np.matrix(Scov)
      ImatS = matS.I
      self.INV.append(ImatS)
      Aname = fname+'_avts_s'+str(frag).zfill(3)+'.npy'
      Avecs = svm.Load(Aname)
      Amat =  np.matrix(Avecs)
      self.AVE.append(Amat)
      Ename = fname+'_expect_s'+str(frag).zfill(3)+'.npy'
...
def main(set_n, trace_number=100):
  ...
  for frag in range(0, 16):
    KEY_Ans.append(svm.Load(('intermediate_values/intermediate_S_KEY/KEY_s'+str(frag).zfill(3)+'.npy'))[(set_n*Set_Size):(set_n*Set_Size+Set_Size)])
    K_A_Ans.append(svm.Load(('intermediate_values/intermediate_S_K_A/K_A_s'+str(frag).zfill(3)+'.npy'))[(set_n*Set_Size):(set_n*Set_Size+Set_Size)])
    K_B_Ans.append(svm.Load(('intermediate_values/intermediate_S_K_B/K_B_s'+str(frag).zfill(3)+'.npy'))[(set_n*Set_Size):(set_n*Set_Size+Set_Size)])
    L_A_Ans.append(svm.Load(('intermediate_values/intermediate_S_L_A/L_A_s'+str(frag).zfill(3)+'.npy'))[(set_n*Set_Size):(set_n*Set_Size+Set_Size)])
    L_B_Ans.append(svm.Load(('intermediate_values/intermediate_S_L_B/L_B_s'+str(frag).zfill(3)+'.npy'))[(set_n*Set_Size):(set_n*Set_Size+Set_Size)])
    T_A_Ans.append(svm.Load(('intermediate_values/intermediate_S_T_A/T_A_s'+str(frag).zfill(3)+'.npy'))[(set_n*Set_Size):(set_n*Set_Size+Set_Size)])
    T_B_Ans.append(svm.Load(('intermediate_values/intermediate_S_T_B/T_B_s'+str(frag).zfill(3)+'.npy'))[(set_n*Set_Size):(set_n*Set_Size+Set_Size)])

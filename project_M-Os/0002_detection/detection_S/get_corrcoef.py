...
class Scatter_Model:
  def __init__(self):
    ...
    for t in range(0, SETS):
      fname = '../Samples/Samples_DN_'+str(t).zfill(4)+'.hdf5'
      print('Loading '+fname)
      FILE = h5py.File(fname, 'r')
      self.Samples.append(FILE['detects'][()])
      FILE.close()
...
def detect(MODEL, TAG, lower, upper):
  for ints in range(lower, upper):
    ...
    for byte in range(4*ints, 4*ints+4):
      BYTE_TAG = TAG+'_s'+str(byte).zfill(3)
      print(BYTE_TAG)
      intermediate_bits.append(svm.Load('intermediate_values/intermediate_S_'+TAG+'/'+BYTE_TAG+'.npy'))
    Scores = MODEL.get_values(intermediate_bits)
    for bt in range(0, 4):
      byte = ints*4+bt
      fname = './detect_results_08/'+TAG+'_r_squ_s'+str(byte).zfill(3)+'.npy'
...
if __name__=='__main__':
  model = Scatter_Model()
  detect(model,'L_A', 0, 4)
  detect(model,'L_B', 0, 4)

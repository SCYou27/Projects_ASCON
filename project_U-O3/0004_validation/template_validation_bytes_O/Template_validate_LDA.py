...
def main(set_n, trace_number=1000):
  ...
  for byte in range(0, 40):
    for rd in range(0, 12):
      temp_Ans = svm.Load(('intermediate_values/intermediate_B_I_A'+str(rd).zfill(2)+'/I_A'+str(rd).zfill(2)+'_b'+str(byte).zfill(3)+'.npy'))
      I_A_ANS[rd] = np.vstack([I_A_ANS[rd], temp_Ans[(set_n*Set_Size):(set_n*Set_Size+Set_Size)]])
      temp_Ans = svm.Load(('intermediate_values/intermediate_B_I_B'+str(rd).zfill(2)+'/I_B'+str(rd).zfill(2)+'_b'+str(byte).zfill(3)+'.npy'))
      I_B_ANS[rd] = np.vstack([I_B_ANS[rd], temp_Ans[(set_n*Set_Size):(set_n*Set_Size+Set_Size)]])
      temp_Ans = svm.Load(('intermediate_values/intermediate_B_F_A'+str(rd).zfill(2)+'/F_A'+str(rd).zfill(2)+'_b'+str(byte).zfill(3)+'.npy'))
      F_A_ANS[rd] = np.vstack([F_A_ANS[rd], temp_Ans[(set_n*Set_Size):(set_n*Set_Size+Set_Size)]])
      temp_Ans = svm.Load(('intermediate_values/intermediate_B_F_B'+str(rd).zfill(2)+'/F_B'+str(rd).zfill(2)+'_b'+str(byte).zfill(3)+'.npy'))
      F_B_ANS[rd] = np.vstack([F_B_ANS[rd], temp_Ans[(set_n*Set_Size):(set_n*Set_Size+Set_Size)]])
    temp_Ans = svm.Load(('intermediate_values/intermediate_B_F_INP/F_INP_b'+str(byte).zfill(3)+'.npy'))
    F_INP_Ans = np.vstack([F_INP_Ans, temp_Ans[(set_n*Set_Size):(set_n*Set_Size+Set_Size)]])
    if byte>=16:
      continue
    temp_Ans = svm.Load(('intermediate_values/intermediate_B_KEY/KEY_b'+str(byte).zfill(3)+'.npy'))

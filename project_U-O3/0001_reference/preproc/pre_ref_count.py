...
def read_wave(input_name):
  ...
  for t in range(0, GROUP_SIZE):
    ...
    Traces.append(Traces_int[t]*Gains[t]+Offsets[t])
  return np.vstack(Traces), np.count_nonzero(R==CT)==GROUP_SIZE

def ref_calculate(L, U):
  Traces = []
  for Set_n in range(L, U):
    trace_block, check = read_wave('../Raw/Raw_RE_'+str(Set_n).zfill(4)+'.hdf5')
    if check:
      print("Response checking passed.")
    else:
      print("Response checking failed.")
      exit()
...
def corr_calculate(set_size, set_num):
  ...
  for Num in range(0, set_num):
    trace_block, check = read_wave('../Raw/Raw_RE_'+str(Num).zfill(4)+'.hdf5')
...
def show_statistics(set_size, set_num, Times):
  ...
  print("=================================================================")
  print("Traces outside of the interval:")

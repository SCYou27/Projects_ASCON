...
class Template:
  def __init__(self, func):
    ...
    self.ICs = []
    fname = 'templateLDA_O'+BOUND+'_16bit/template_'+func+'/template'
    self.INV = []
    self.EXP = []
    self.AVE = []
    ...
    for frag in range((4*lower), (4*upper)):
  ...
  def Guess(self, Trace):
    ...
    for lane in range(0, 2):
      ...
      for frag in range(4*lane, 4*lane+4):
        ...
        ImatS = self.INV[frag]
        matX = np.matrix(np.ones((65536, 1)))*Xm - self.EXP[frag]
        pos = -0.5*abs(np.sum(np.array(matX*ImatS)*np.array(matX), axis=1))
        Prob = np.transpose(np.vstack([np.arange(65536.0), pos]))
        Great_Prob_Table.append(Prob)
    return np.array(Great_Prob_Table)

class Template_Recovering:
  def __init__(self):
    print('Template initalization')
    self.Template_KEY = Template('KEY')
    self.Template_OUT = Template('F_B11')
    return
  
  def recover(self, trace):
    Guess_KEY = self.Template_KEY.Guess(trace)
    Guess_OUT = self.Template_OUT.Guess(trace)
    return Guess_KEY, Guess_OUT
